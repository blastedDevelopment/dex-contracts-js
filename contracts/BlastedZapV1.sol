...
import {IBlastedPair} from "./interfaces/IBlastedPair.sol";
import {IBlastedRouter02} from "./interfaces/IBlastedRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by BlastedSwap teams.
 */
contract BlastedZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // BlastedRouter interface
    IBlastedRouter02 public pancakeRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from BlastedSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address BlastedRouter
     ...
     * @param _pancakeRouter: address of the BlastedRouter
    ...
    constructor(
        ...
        pancakeRouter = IBlastedRouter02(_pancakeRouter);
    ...
    {
        address token0 = IBlastedPair(_lpToken).token0();
        address token1 = IBlastedPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IBlastedPair(_lpToken).getReserves();
    ...
    {
        require(
            _token0ToZap == IBlastedPair(_lpToken).token0() || _token0ToZap == IBlastedPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IBlastedPair(_lpToken).token0() || _token1ToZap == IBlastedPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IBlastedPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IBlastedPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IBlastedPair(_lpToken).totalSupply();
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IBlastedPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "BlastedRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "BlastedRouter: INSUFFICIENT_B_AMOUNT");
