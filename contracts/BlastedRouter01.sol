...
import "./interfaces/IBlastedRouter01.sol";
import "./interfaces/IBlastedFactory.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/IWETH.sol";

contract BlastedRouter01 is IBlastedRouter01 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = BlastedLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = BlastedLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                ...
                uint amountAOptimal = BlastedLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IBlastedPair(pair).burn(to);
    ...
    function getAmountIn(
        ...
        return BlastedLibrary.getAmountOut(amountOut, reserveIn, reserveOut);
    }
