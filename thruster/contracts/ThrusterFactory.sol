...
contract ThrusterFactory is IThrusterFactory, ThrusterGas {
    ...
    constructor(
        address _yieldToSetter,
        address _pointsAdmin
    ) public ThrusterGas(_yieldToSetter) {
    ...
    function setYieldToSetter(address _newYieldToSetter) external {
        require(msg.sender == yieldToSetter, "ThrusterFactory: FORBIDDEN");
        require(
            _newYieldToSetter != address(0),
            "ThrusterFactory: ZERO_ADDRESS"
        );
    ...
    function setYieldCut(uint256 _yieldCut) external {
        require(msg.sender == yieldToSetter, "ThrusterFactory: FORBIDDEN");
        require(
            _yieldCut < 6 && _yieldCut > 0,
            "ThrusterFactory: INVALID_YIELD_CUT"
        ); // Yield cut is 16-50% of the LP yield
        yieldCut = _yieldCut;
