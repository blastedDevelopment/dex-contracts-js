...
import "./ThrusterGasRouter.sol";
import "../libraries/SafeMath.sol";
import "../libraries/TransferHelper.sol";
import "../libraries/ThrusterLibrary.sol";
...
contract ThrusterRouter is IThrusterRouter02, ThrusterGasRouter {
    ...
    constructor(
        address _factory,
        address _WETH
    ) public ThrusterGasRouter(IThrusterFactory(_factory).yieldToSetter()) {
        factory = _factory;
        WETH = _WETH;
        IBlastPoints(BLAST_POINTS).configurePointsOperator(
            IThrusterFactory(_factory).pointsAdmin()
        );
    }
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = ThrusterLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = ThrusterLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "ThrusterRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = ThrusterLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "ThrusterRouter: INSUFFICIENT_A_AMOUNT"
                );
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? ThrusterLibrary.pairFor(factory, output, path[i + 2])
                : _to;
        ...
        returns (uint256[] memory amounts)
    {
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            ThrusterLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
        ...
        returns (uint256[] memory amounts)
    {
        ...
        require(
            ...
            "ThrusterRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = ThrusterLibrary.sortTokens(input, output);
            IThrusterPair pair = IThrusterPair(
                ThrusterLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = ThrusterLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
