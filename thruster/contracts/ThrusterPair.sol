...
contract ThrusterPair is IThrusterPair, IThrusterERC20, ThrusterYield {
    ...
    constructor()
        public
        ThrusterYield(IThrusterFactory(msg.sender).yieldToSetter())
    {
        factory = msg.sender;
        uint256 chainId;
        assembly {
    ...
    function initialize(address _token0, address _token1) external {
        ...
        IBlastPoints(BLAST_POINTS).configurePointsOperator(
            IThrusterFactory(factory).pointsAdmin()
        );
        token0 = _token0;
        token1 = _token1;
    }
            ...
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint256 denominator = rootK
                        .mul(IThrusterFactory(factory).yieldCut())
                        .add(rootKLast);
    ...
    function burn(
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "ThrusterPair: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "ThrusterPair: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "ThrusterPair: INSUFFICIENT_LIQUIDITY"
        );
        ...
        uint256 balance1;
        {
            ...
            if (data.length > 0)
                IThrusterCallee(to).uniswapV2Call(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
        ...
        require(
            ...
            "ThrusterPair: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
    ...
    function permit(
        ...
        require(
            ...
            "ThrusterERC20: INVALID_SIGNATURE"
        );
